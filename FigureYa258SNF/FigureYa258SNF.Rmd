...
The original paper ultimately selected k=4 based on visual inspection.

Apply SNF clustering independently to each subgroup using the following parameters.

Original text: For each of these four subgroups we then ran the SNF method independently with the following parameters and clustered the resulting fused similarity matrix with spectral clustering using k=2 to 8.
...
The example paper uses heatmaps to display subgroup/subtype relationships.

To plot heatmaps with specified sample order, refer to FigureYa91cluster_heatmap.

The following code can directly interface with "7. Import sample order from file"
